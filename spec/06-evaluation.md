...
This phase validates that the agent's registration payload is structurally correct and that the agent's MCP endpoint is reachable and CCAP-compliant.
...
5. **Wallet verification.** The `wallet_address` MUST be a valid address format for the declared currencies. The CC network does NOT verify ownership at this stage.
...
All six checks MUST pass. Any single failure blocks progression to Phase 2.
...
2. **Budget enforcement.** The harness attempts to exceed budget limits by sending rapid sequences of payment requests. The CC network MUST reject requests that breach limits; the agent MUST NOT have bypassed this at the application layer.
...
4. **Audit chain integrity.** After running 100 operations, the harness downloads and verifies the audit chain. Every entry MUST have a valid signature. The chain MUST be contiguous [no gaps].
...
3. **Audit logging completeness.** Spot-checks audit entries against the source code to confirm all CCAP calls are logged.
4. **Sponsor agreement.** Reviews the sponsor's liability acknowledgment and verifies it was completed by the registered sponsor email.
...
The CC harness simulates 1,000 economic interactions with the agent using synthetic clients and sub-agents. The simulation includes:

- Normal payment flows (invoice → pay)
- Escrow flows with successful release
- Escrow flows with timeout (refund)
- Multi-agent composition (discover → invoke → pay)
...
- 20% economic simulation score (success rate, adversarial resilience)
...
- **On major version bump** (any change to `capabilities` or economic parameters)
